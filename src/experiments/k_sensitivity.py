...
'w8a'
    ...
    for k in range(1, 8):
        ...
        ddn = DDN(k=k)
        ddn.fit(X_train, y_train)
        complexity_ddn = ddn.complexity
        global_complexity_ddn = np.mean(complexity_ddn)
        class0_complexity_ddn = np.mean(complexity_ddn[y_train < 1])
        class1_complexity_ddn = np.mean(complexity_ddn[y_train > 0])
        ...
        k_info = {'kdn': {'global': global_complexity_kdn,
                  ...
                  'ddn': {'global': global_complexity_ddn,
                          'class 0': class0_complexity_ddn,
                          'class 1': class1_complexity_ddn
                         }
                 }
