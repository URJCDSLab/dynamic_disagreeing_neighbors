import pandas as pd
import numpy as np
from scipy.stats import ttest_rel, wilcoxon, shapiro, spearmanr, f_oneway, kruskal, levene, bartlett
...
def calculate_grouped_statistics(df, diff='diff_score_most_complex_class', x_var='best_method'):
    ...
    try:
        grouped_stats = df.groupby([x_var, 'metric_y'])[diff].agg(['median','mean', 'std', 'min', 'max', 'count']).reset_index()
         ...
         grouped_stats = df.groupby([x_var])[diff].agg(['median','mean', 'std', 'min', 'max', 'count']).reset_index()
...
def perform_variance_tests(df, remove_outliers=False, return_outliers=False):
    """
    Perform variance tests (Levene's or Bartlett's) on specified columns of the DataFrame 
    with an option to remove outliers and return them.
    ...
    def interpret_significance(p_value):
        ...
        """
        if p_value < 0.01:
            return "The difference in variance is highly significant at the 99% confidence level (p < 0.01)."
        elif p_value < 0.05:
            return "The difference in variance is significant at the 95% confidence level (p < 0.05)."
        elif p_value < 0.10:
            return "The difference in variance is moderately significant at the 90% confidence level (p < 0.10)."
        else:
            return "The difference in variance is not statistically significant."
    ...
    # Perform variance tests for each metric
    for metric in df['metric'].unique():
        metric_data = df[df['metric'] == metric]
        print(f"\nResults for measure: {metric}")
        ...
        for col_global, col_mean_folds in columns_to_test:
            ...
            if shapiro_test_global.pvalue > 0.05 and shapiro_test_mean_folds.pvalue > 0.05:
                # Both distributions are normal, perform Bartlett's test
                stat, p_val = bartlett(data_global, data_mean_folds)
                test_name = "Bartlett's test"
            else:
                # Use Levene's test, more robust to non-normal data
                stat, p_val = levene(data_global, data_mean_folds)
...
def compare_variability_between_metrics(df):
    """
    Compare the variability (variance) between the metrics 'kdn' and 'ddn' for specified columns.

    Parameters:
    -----------
    df : pandas.DataFrame
        The DataFrame containing the data to be tested. It must include the columns specified in `columns_to_test` 
        and a 'metric' column that includes 'kdn' and 'ddn'.

    Returns:
    --------
    None
        Prints the results of the Levene's test for each column and metric comparison.
    """
    # Define the columns that represent differences to test
    columns_to_test = [
    ...
    # Perform variance comparison for each variable in columns_to_test
    for col in columns_to_test:
        kdn_data = df[df['metric'] == 'kdn'][col]
        ddn_data = df[df['metric'] == 'ddn'][col]

        # Perform Levene's test for variance comparison between kdn and ddn
        stat, p_val = levene(kdn_data, ddn_data)
        significance = interpret_significance(p_val)

        print(f"Comparison: {col} - Levene's test between 'kdn' and 'ddn':")
        print(f"Statistic: {stat:.3f}, p-value: {p_val:.3f}\n{significance}\n")

from scipy.stats import ttest_ind, mannwhitneyu, shapiro

def compare_means_between_metrics(df):
    """
    Compare the means between the metrics 'kdn' and 'ddn' for specified columns.
    ...
    --------
    None
        Prints the results of the t-test or Mann-Whitney test for each column and metric comparison.
    """
    # Define the columns that represent differences to test
    columns_to_test = [
        'dataset_complexity_difference',
    ...
    def interpret_significance(p_value):
        ...
        """
        if p_value < 0.01:
            return "The difference in means is highly significant at the 99% confidence level (p < 0.01)."
        elif p_value < 0.05:
            return "The difference in means is significant at the 95% confidence level (p < 0.05)."
        elif p_value < 0.10:
            return "The difference in means is moderately significant at the 90% confidence level (p < 0.10)."
        else:
            return "The difference in means is not statistically significant."

    # Perform means comparison for each variable in columns_to_test
    for col in columns_to_test:
        kdn_data = df[df['metric'] == 'kdn'][col]
        ddn_data = df[df['metric'] == 'ddn'][col]

        # Test for normality using Shapiro-Wilk test
        shapiro_kdn = shapiro(kdn_data)
        shapiro_ddn = shapiro(ddn_data)

        if shapiro_kdn.pvalue > 0.05 and shapiro_ddn.pvalue > 0.05:
            # Both distributions are normal, perform independent t-test
            stat, p_val = ttest_ind(kdn_data, ddn_data)
            test_name = "Independent t-test"
        else:
            # Distributions are not normal, perform Mann-Whitney U test
            stat, p_val = mannwhitneyu(kdn_data, ddn_data)
